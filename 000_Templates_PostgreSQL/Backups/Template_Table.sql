...
ALTER TABLE <tableName> 
    DROP CONSTRAINT <tableName>_status; 

ALTER TABLE <tableName> 
   ADD CONSTRAINT <tableName>_status CHECK (status IN ('C', 'D', 'E', 'H', 'L', 'N', 'P', 'Q', 'R', 'W')); 
...
BEGIN 
    ...
    IF (TG_OP='INSERT') THEN 
        ...
        INSERT INTO <tableName>_audit 
            (rec_id, cmd, update_date, update_by) --, o_<tableName>_nm)  
        SELECT NEW.rec_id, 'I', NOW(), USER; --, OLD.<tableName>_nm;
    
    ELSIF (TG_OP='UPDATE') THEN 
        
        INSERT INTO <tableName>_audit 
            (rec_id, cmd, update_date, update_by, n_<tableName>_nm, o_<tableName>_nm)  
        SELECT NEW.rec_id, 'U', NOW(), USER, NEW.<tableName>_nm, OLD.<tableName>_nm;
    
    ELSIF (TG_OP='DELETE') THEN 
        
        INSERT INTO <tableName>_audit 
            (rec_id, cmd, update_date, update_by, o_<tableName>_nm)  
        SELECT OLD.rec_id, 'D', NOW(), USER, OLD.<tableName>_nm;
