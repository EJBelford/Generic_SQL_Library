...
-- DROP SEQUENCE EnumTypeRef_seq;

CREATE SEQUENCE EnumTypeRef_seq
--    START WITH 1000000 
    MINVALUE   1
    NOCYCLE
    NOCACHE
...
DROP TABLE EnumTypeRef CASCADE CONSTRAINTS;

CREATE TABLE EnumTypeRef (
...
COMMENT ON TABLE EnumTypeRef IS
    'EnumTypeRef - The overarching data object that represents whole enumerations in the database by their names.';


/*----- Column Meta-Data -----*/ 

COMMENT ON COLUMN EnumTypeRef.rec_id IS
    'rec_id - Primary, blind key of the EnumTypeRef table.';

COMMENT ON COLUMN EnumTypeRef.rec_uuid IS
    'rec_uuid - Blind uuid key of the EnumTypeRef table.';

COMMENT ON COLUMN EnumTypeRef.enum_type IS
    'ENUM_TYPE - Identifying name of the enumeration in the database.  ';

COMMENT ON COLUMN EnumTypeRef.enum_type_desc IS
    'ENUM_TYPE_DESC - Brief description of the enumeration.  ';

COMMENT ON COLUMN EnumTypeRef.classification IS
...
COMMENT ON COLUMN EnumTypeRef.constant_flag IS
...
COMMENT ON COLUMN EnumTypeRef.source_name IS
    'source_name - The source of the enumeration type. ';
    
COMMENT ON COLUMN EnumTypeRef.source_version IS
    'source_version - Unquie identifier used to designate a specific version od enum type. ';

COMMENT ON COLUMN EnumTypeRef.status IS
...
CREATE UNIQUE INDEX pk_EnumTypeRef ON
    EnumTypeRef (
        enum_type
    ASC )
...
ALTER TABLE EnumTypeRef
    ADD CONSTRAINT ixu_enumtyperef_recuuid UNIQUE ( recuuid )
        USING INDEX PCTFREE 10 INITRANS 2 TABLESPACE Generic_Dev_TabSpace
            STORAGE ( INITIAL 65536 NEXT 1048576 PCTINCREASE 0 MINEXTENTS 1 MAXEXTENTS 2147483645 FREELISTS
1 FREELIST GROUPS 1 BUFFER_POOL DEFAULT )
        COMPUTE STATISTICS;
...
CREATE OR REPLACE TRIGGER TR_I_ENUMTYPEREF_SEQ 
    BEFORE INSERT ON EnumTypeRef REFERENCING 
    NEW AS New 
    OLD AS Old 
    FOR EACH ROW 
